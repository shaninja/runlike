...
def missing_target_metadata_check(root):
    root = Path(root)
    return {
        "expected": "Generated by `make refresh-support-artifacts`.\n",
        "path": root / "spec" / "current-target.json",
    }
...
def build_checks(root=ROOT):
    ...
    if not target_path.exists():
        return [missing_target_metadata_check(root)]
    ...
    matrix = matrix_module.build_support_matrix(
        ...
        target=matrix_module.load_target(target_path))
